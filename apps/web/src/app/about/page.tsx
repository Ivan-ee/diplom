  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-12">
      ...
      <section className="grid grid-cols-1 sm:grid-cols-3 gap-8 mb-16">
        {[
          ...
          <div key={title} className="text-center">
            <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-warm-ivory">
              <Icon size={24} className="text-caramel" />
            </div>
            <h3 className="font-heading text-lg font-semibold text-graphite mb-2">{title}</h3>
            <p className="text-sm text-graphite-light">{text}</p>
          </div>
        ))}
      </section>
      ...
      <section className="bg-warm-ivory rounded-2xl p-8 sm:p-12 mb-16">
        <h2 className="font-heading text-2xl font-semibold text-graphite mb-4">Наша история</h2>
      ...
      {/* Contacts */}
      <section>
        <h2 className="font-heading text-2xl font-semibold text-graphite mb-6">Контакты</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div className="flex flex-col gap-4">
            {shopConfig.phones.map((phone) => (
              <div key={phone.value} className="flex items-start gap-3">
                <Phone size={20} className="text-caramel mt-0.5 shrink-0" />
                <div>
                  <p className="font-medium text-graphite">{phone.label}</p>
                  <a
                    href={`tel:${phone.value}`}
                    className="text-sm text-caramel hover:text-caramel-hover transition-colors"
                  >
                    {phone.display}
                  </a>
                </div>
          ...
          <div className="flex items-start gap-3">
            <svg
              ...
              fill="currentColor"
              aria-hidden="true"
              className="text-caramel mt-0.5 shrink-0"
            >
            ...
            <div>
              <p className="font-medium text-graphite">ВКонтакте</p>
              <a
