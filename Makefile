...
.PHONY: help install \
        ...
        docker-infra docker-up docker-down docker-restart docker-logs \
        db-generate db-migrate db-push db-seed db-studio db-reset \
        assets-upload photos-upload db-seed-full \
        clean
...
dev: docker-infra ## Поднять инфру и запустить все приложения в dev-режиме
	@$(PNPM) dev
...
docker-infra: ## Запустить только инфраструктуру (postgres, minio, meilisearch) в фоне
	@$(COMPOSE) up -d postgres minio minio-init meilisearch

docker-up: ## Запустить весь локальный Docker-стек (infra + api + web)
	@$(COMPOSE) up -d --build
...
assets-upload: ## Upload seed photos and constructor models to MinIO
	@$(PNPM) --filter @bakery/db assets:upload

photos-upload: assets-upload ## Backward-compatible alias for assets-upload

db-seed-full: docker-infra ## Full seed: Docker infra + migrations + assets + DB data
	@$(COMPOSE) --profile seed run --rm --build seed
	@if $(COMPOSE) ps --all --services | grep -qx api; then \
		$(COMPOSE) up -d --build --no-deps api; \
	else \
		echo "API container is not running; it will reindex search on next start."; \
	fi
